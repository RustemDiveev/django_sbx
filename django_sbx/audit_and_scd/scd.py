...
class BaseAuditModel(models.Model):
    ...
    def get_data_dict(self, before_changes:bool=False) -> dict:
        ...
        if before_changes:
            data_obj = self.objects.get(pk=self.id)
        else:
            data_obj = self

        for field in self.get_fields():
            data.update({field: getattr(data_obj, field)})
        return data
...
def get_hardcoded_model():
    ...
    meta_attributes = {
        ...
        "constraints": [
            models.UniqueConstraint(fields=["pk_field"], name="SCDModelV1_dummy_pk")
        ]
    }
    ...
    return type("SCDModelV1", (BaseAuditModel,), attributes)
